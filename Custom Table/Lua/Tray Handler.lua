...
function onload()
    ...
    -- Load playerHand Lua
    WebRequest.get(
        "https://raw.githubusercontent.com/Tagischeap/TableTopSimulator/master/Custom%20Table/Fetch/PlayerHand.lua",
        function(request)
    ...
    WebRequest.get(
        "https://raw.githubusercontent.com/Tagischeap/TableTopSimulator/master/Custom%20Table/Fetch/HandCounter.lua",
        function(request)
            if request.is_error then
                log(request.error)
            else
    ...
    WebRequest.get(
        "https://raw.githubusercontent.com/Tagischeap/TableTopSimulator/master/Custom%20Table/Fetch/LifeCounter.lua",
        function(request)
            if request.is_error then
                log(request.error)
            else
...
function spawnCircles()
    if ply ~= nil then removeCircles() end
...
function removeCircles()
    for i, v in pairs(ply) do v.destruct() end
    ply = {}
end
...
function circlePoint(i)
    ...
    return {
        position = vector(roundNumberToFactor(ptx, 15), 0.5,
                          roundNumberToFactor(pty, 15)),
...
function setup()
    ...
    -- Get seated players once and store in a lookup table
    local seatedPlayers = {}
    for _, color in pairs(getSeatedPlayers()) do seatedPlayers[color] = true end

    -- Calculate the number of unseated colors and the centered starting z position
    local unseatedCount = #Player.getColors() - #getSeatedPlayers() - 2
    local spacing = 30
    local startZ = -(spacing * (unseatedCount - 1) / 2)
    local num = 0

    -- Loop through each player color
    for _, color in ipairs(Player.getColors()) do
        -- Check if color is unseated and not special colors ("Grey", "Black")
        if color ~= "Grey" and color ~= "Black" and not seatedPlayers[color] then
            num = num + 1
            -- Move each unseated color to the centered line on the z-axis
            Player[color].setHandTransform({
                position = {x = -100, y = 10, z = startZ + (num - 1) * spacing},
                rotation = {0, 90, 0}
            })
        end
    end
    ...
    for i, v in pairs(mats) do
        ...
        if v ~= "Grey" and v ~= "Black" then
            ...
            for a, b in pairs(stuff) do
                ...
                local fixpos = {
                    tr.position[1] + math.cos(rotation_radians) * pos.x +
                        math.sin(rotation_radians) * pos.z, pos.y,
                    tr.position[3] - math.sin(rotation_radians) * pos.x +
                        math.cos(rotation_radians) * pos.z
                }

                -- Calculate the new rotation
                local fixrot = {
                    tr.rotation[1] + rot.x, tr.rotation[2] + rot.y,
                    tr.rotation[3] + rot.z
                }
...
function spawnSet() -- Spawns a set
    ...
    for i = 0, 10 do
        data = "-t%3Aland+in%3Abooster+set%3A" .. set ..
                   "+-is%3Adoublefaced+r%3Ac+c%3A"
        ...
        elseif i == 6 then -- UnCommon
            data = "-t%3Abasic+in%3Abooster+set%3A" .. set ..
                       "+-is%3Adoublefaced+r%3Au"
...
function spawnEmptyPack() return getObjectFromGUID("987b7c").clone() end
...
function positionSeat(a, p)
    for o, l in pairs(p) do
        ...
        for a, b in pairs(l[2]) do
            ...
            -- Calculate the new position
            local fixpos = {
                nr.position[1] + math.cos(rotation_radians) * pos.x +
                    math.sin(rotation_radians) * pos.z, pos.y,
                nr.position[3] - math.sin(rotation_radians) * pos.x +
                    math.cos(rotation_radians) * pos.z
            }
            -- Calculate the new rotation
            local fixrot = {
                nr.rotation[1] + rot.x, nr.rotation[2] + rot.y,
                nr.rotation[3] + rot.z
            }
...
function spawnHandCounter(p)
    ...
    obj.setCustomObject({
        image = "https://steamusercontent-a.akamaihd.net/ugc/1646593716898448966/68DC98F323399C7CADF7E53933EAC99FBB94167E/",
        thickness = 0.15,
        sound = false,
        snap_to_grid = true,
        ignore_fog_of_war = true
...
function spawnLifeCounter(p)
    ...
    obj.setCustomObject({
        image = "https://steamusercontent-a.akamaihd.net/ugc/1836915738113613310/922E11FF5CBC801148AA87DE60ED64472A618F3D/",
        collision = false,
        thickness = 0.15,
        sound = false,
        snap_to_grid = true,
...
function spawnCommanderDamage(p) -- Spawns Commander Damage Counter
    ...
    obj.setCustomObject({
        image = "https://steamusercontent-a.akamaihd.net/ugc/1009313481328783631/DDCC83524ED154C1F937AC5319E4903077E4A335/",
        collision = false,
        thickness = 0.15,
        sound = false,
        snap_to_grid = true,
...
function findSeats(a, i) -- Amount of players, Player number
    ...
    if i ~= nil and a ~= nil then
        off = 0
        x =
            ((44 * math.ceil((i - 2) / 4)) * (-1 + ((math.ceil(i / 2) % 2) * 2)))
        y = 14 - (28 * (i % 2))
        -- print(i%2)
        -- Rotate ends to face
